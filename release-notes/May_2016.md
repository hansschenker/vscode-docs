...
The May release of VS Code has some great new features, enhancements to existing features, and a set of important bug fixes. Here are the highlights:
...
* **Workbench**: A new integrated Terminal allows you to stay in Code while using your platform's shell, we've added the ability to remove key bindings, and we've made progress on Tab support
...
The editor automatically inserts whitespace when you add a new line, based on indenting rulesa and the previous line's content. Over time this can result in unnecessary whitespace being persisted in the file. Thanks to [PR #5423](https://github.com/Microsoft/vscode/pull/5423) by [@outcoldman](https://github.com/outcoldman), the new `editor.trimAutoWhitespace` setting causes the editor to track the automatic whitespace it inserted and remove it if it no longer is needed, for example by typing somewhere else. Please note, this new setting is on by default.

![Trim Auto Whitespace](images/May/trimAutoWhitespace.gif)

### Select all find matches

You can now immediately select all find results by pressing `kb(editor.action.selectAllMatches)`. This is much quicker than doing a find and replace and leverages VS Code's multi-cursor support. Thanks to [@Inori](https://github.com/Inori) for [PR #5715](https://github.com/Microsoft/vscode/pull/5715)!

![Find All Matches](images/May/selectAllFindMatches.gif)
...
[Tab support](https://github.com/Microsoft/vscode/issues/224) is the number one [User Voice request](https://visualstudio.uservoice.com/forums/293070-visual-studio-code/suggestions/7752519-implement-tabs) and it will take multiple iterations to fully implement.

In May we made great progress on how to manage *stacks* of open editors. In the picture below you can see that the left pane has two editors open, the center pane has four, and the right pane has two more. Files listed in italics are *preview* files, they allow you to single click and browse files without cluttering the list of open editors and eventually tabs. 

![Tabs](images/May/tabs.png)

When you close an editor the last opened editor in the stack is restored, instead of closing the pane as it does today. In addition, we held a number of UX studies that have resulted in further improvments to the design, including:

* Redesign of the overflow icon
* Ability to specify if files from **Quick Open** are pinned or previewd
* Added a command to turn a previewed file into a pinned file

There is **much** more work to be done but we do hope to start moving some of this to master in June.    
...
With the May release we are rolling out the initial version of an integrated terminal. You can open the terminal using `kb(workbench.action.terminal.toggleTerminal)`, from the **View** | **Toggle Terminal** menu, or from the **View** > **Toggle Terminal** command in the **Command Palette**.
...
![Terminal](images/May/integrated-terminal.png)
...
The [TSLint extension](https://marketplace.visualstudio.com/items?itemName=eg2.tslint) now supports excluding files from linting ([PR #47](https://github.com/Microsoft/vscode-tslint/pull/47)) as well as quix fixes for a set of TSLint warnings ([PR #40](https://github.com/Microsoft/vscode-tslint/pull/40)). Thanks for the pull requests! 
...
![100 Local Variables](images/May/many-locals.png)
...
To make it easier to automate and configure VSCode, it is now possible to list, install, and uninstall extensions from the command line. 
...
Our goal is to enable anyone to consume the "Monaco" editor and host it in any browser. The challenge we have today is that the editor is embedded in the `vscode` repostiory and the language service extensions have  dependencies on client (non browser) technologies. 
